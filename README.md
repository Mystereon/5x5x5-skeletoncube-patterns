...
![Platform](https://img.shields.io/badge/platform-Arduino-00979D) ![LEDs](https://img.shields.io/badge/LEDs-125%20WS2812B-brightgreen) ![CubeFX](https://img.shields.io/badge/CubeFX%20modes-145-purple) ![Library](https://img.shields.io/badge/catalogue-228%20entries-purple) ![License](https://img.shields.io/badge/license-MIT-blue)
...
apps/                                  # Native Android companion source projects
  CubeFXPhone/                         # 145-mode Android phone controller
...
[`standalone/CubeFXWeb`](standalone/CubeFXWeb/) is a complete ESP32 Wi-Fi and BLE controller inspired by the fast, friendly control approach of [WS2812FX](https://github.com/kitesurfer1404/WS2812FX). It starts as a local Wi-Fi access point, serves its own cube-aware web UI, displays a live isometric voxel preview, and now exposes **145 live selectable CubeFX modes**. The current enclosure profile targets the Waveshare ESP32-S3-Zero and drives **137 LEDs** over one data chain: **0–124 remain the 5×5×5 cube, while 125–136 are a rear-facing 12-pixel enclosure mood ring**. Its browser includes a **five-slot / sixteen-step User Sequencer**: draw through five tappable 5×5 `z`-layer panels, set each step’s RGB colour and duration, then deliberately upload a staged sixteen-step slot. Modes 58–62 play those persistent user slots. Modes 63–145 are **separately labelled CubeFX interpretations** of all 83 official FastLED source directories. The pure source itself remains unchanged under [`third_party/FastLED`](third_party/FastLED), while [`FASTLED_COLLECTION.md`](FASTLED_COLLECTION.md) gives the exact one-to-one provenance map. No CubeFX interpretation is presented as an original FastLED example.
...
The current CubeFXWeb selector has **145 embedded modes**: 57 native CubeFX scenes, five persistent User Pattern slots, and 83 clearly labelled CubeFX interpretations. The FastLED submodule adds 83 untouched official source entries, making the public catalogue floor **228 entries** before later additions. CubeFXPhone exposes the matching **145 direct-play BLE selections**, with direct Next Pattern, full everyday LIVE controls, useful firmware/wiring/Watch/support routes, and an Owner Easter Eggs deck.
...
The native Kotlin/Jetpack Compose source for the Android phone controller and the Galaxy Watch8 Classic companion is in [`apps/`](apps/). The phone app offers all **145 embedded CubeFXWeb modes**, including User Pattern BLE IDs **75–79** and the separately labelled FastLED interpretation range **80–162**. Its explicit opt-in **Audio Link** requests microphone permission, analyses audio locally, and streams only a small spectrum envelope while active. It also supplies live engine/Banner controls, an ESP32 wiring-and-dimensions setup screen, and a Watch route. Build and installation notes are in [`apps/README.md`](apps/README.md).
...
The Waveshare ESP32-S3-Zero uses an ESP32-S3FH4R2 with **4 MB flash, 2 MB PSRAM, and 512 KB internal SRAM**. Select **ESP32S3 Dev Module**, **4 MB flash**, **Huge APP (3 MB No OTA / 1 MB SPIFFS)**, and **QSPI PSRAM** in Arduino IDE. The enclosure mapping is GPIO6 data, GPIO2 primary button, and GPIO4 secondary button. The present **145-mode, 137-output** build compiles to **1,352,003 bytes flash (42% of the 3 MB app slot)** and **55,832 bytes static RAM (17% of the reported internal-RAM allocation)**. It retains more than 1.7 MB of application flash for future pattern code. The standalone VolumetricEngineDemo compiles separately at **432,343 bytes flash (13%)** and **27,940 bytes global RAM (8%)**.
