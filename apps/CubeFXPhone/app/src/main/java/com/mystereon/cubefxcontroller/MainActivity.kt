...
private fun PatternPage(cubeFx: CubeFxBleClient) {
    ...
    Column(Modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Text("The first 145 entries below are the modes built into CubeFXWeb and every PLAY button sends its canonical BLE ID. Race Circuit is ID 74; IDs 75–79 play five persistent 16-step User Pattern slots authored in the CubeFXWeb sequencer; IDs 80–162 are separately labelled CubeFX interpretations of the preserved official FastLED sources. The remaining 17 are honest standalone reference sketches, kept separate so no visible row silently does nothing.", color = Muted, fontSize = 13.sp)
        ...
        LazyColumn(modifier = Modifier.weight(1f), verticalArrangement = Arrangement.spacedBy(7.dp)) {
            item { Text("145 EMBEDDED CUBEFXWEB MODES", color = Lime, fontSize = 12.sp, fontWeight = FontWeight.Black, modifier = Modifier.padding(top = 4.dp)) }
