...
> **Recommended CubeFX assignment:** LED data = **GPIO18**; Button 1 = **GPIO16**; Button 2 = **GPIO17**.
...
|---|---:|---|---|
| WS2812B cube DIN | **GPIO18** | GPIO18 → 330–470 Ω series resistor → level shifter → cube DIN | A normal bidirectional GPIO, clear of boot strapping, USB/JTAG, onboard RGB, UART labels, and S3-Zero PSRAM wiring. |
| Button 1 / primary | **GPIO16** | Momentary switch from GPIO16 to GND | Normal GPIO using CubeFX's `INPUT_PULLUP`; short press is pattern-primary, long press opens Banner mode. |
| Button 2 / secondary | **GPIO17** | Momentary switch from GPIO17 to GND | Normal GPIO using CubeFX's `INPUT_PULLUP`; short press is pattern-secondary, long press advances to the next pattern and enters manual mode. |
| Cube ground | **GND** | Connect to the cube power supply ground | Required common reference for the data signal. |

Use a **74AHCT125**, **74HCT245**, or similar 5 V-tolerant logic level shifter between GPIO18 and a 5 V WS2812B cube. Do not power 125 LEDs from the S3-Zero board: Waveshare rates its onboard regulator at 800 mA, whereas the cube needs its own properly sized 5 V supply. [1]
...
#define CUBEFX_LED_DATA_PIN       18
#define CUBEFX_PRIMARY_BUTTON_PIN 16
#define CUBEFX_SECONDARY_BUTTON_PIN 17
```
